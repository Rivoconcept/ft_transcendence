...
JWT_SECRET=$(grep JWT_SECRET /run/secrets/vault-init | cut -d= -f2)
REFRESH_SECRET=$(grep REFRESH_SECRET /run/secrets/vault-init | cut -d= -f2)
POSTGRES_PASSWORD=$(grep POSTGRES_PASSWORD /run/secrets/vault-init | cut -d= -f2)
PGADMIN_DEFAULT_PASSWORD=$(grep PGADMIN_DEFAULT_PASSWORD /run/secrets/vault-init | cut -d= -f2)
